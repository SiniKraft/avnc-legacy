...
class VncActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Resize layout when keyboard is shown on pre-21 devices
            ...
            rootView.viewTreeObserver.addOnGlobalLayoutListener {
                ...
                if (keypadHeight > screenHeight * 0.15) {
                    if (rootView.paddingBottom == 0) {
                        // Check orientation to decide if we need to add padding to the right because of navigation bar
                        if (resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
                            val rightPadding = rootView.rootView.width - r.right
                            rootView.setPadding(0, 0, rightPadding, keypadHeight)
                        } else {
    ...
    private fun workarounds(keyEvent: KeyEvent): Boolean {
        ...
        if (keyEvent.keyCode == KeyEvent.KEYCODE_BACK &&
            InputDevice.getDevice(keyEvent.deviceId)?.sources?.and(InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE &&
